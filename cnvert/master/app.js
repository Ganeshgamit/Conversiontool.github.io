// --- State ---
let originalImage = null; // data URL of the originally uploaded image
let workingImage  = null; // data URL of the current image (after edits)
...
function handleUpload(e) {
  ...
  reader.onload = () => {
    originalImage = reader.result; // data URL
...
function handleOption(option) {
  ...
  if (option === 'Convert') {
    ...
    addMessage('bot', '✂️ Crop tool not implemented yet.');
    showPostActionMenu();
...
function processImage({ action, format = 'png', width = null, height = null, quality = 0.9 }) {
  ...
  img.onload = () => {
    ...
    // For resize, keep current format. For convert/compress, use chosen.
    ...
    // Update working image (enables chaining)
...
function downloadCurrent() {
  ...
  addDownloadLink(workingImage, 'current-image');
}
...
// Optional: start in a clean state on load
